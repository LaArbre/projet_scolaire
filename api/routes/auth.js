...
router.post('/register', async (req, res) => {
    try {
        ...
        const [result] = await db.query(
            ...
            [fullname, email, hashedPassword, 'employee']
        );
...
router.post('/login', async (req, res) => {
    try {
        ...
        const [rows] = await db.query(
            'SELECT id, fullname, email, password, role, failed_attempts, locked_until FROM users WHERE email = ?',
        ...
        if (!valid) {
            ...
            if (user.failed_attempts >= 4) {
                ...
                await db.query('UPDATE users SET locked_until = ? WHERE id = ?', [lockUntil, user.id]);
            }
            return res.status(400).json({ error: true, fields: ['email', 'password'] });
        }
